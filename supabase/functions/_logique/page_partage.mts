...
//
// Le token vit dans la chaine de requete, lue cote navigateur. Il est
// donc deja dans les journaux du serveur qui sert cette page : c'est
// une difference assumee avec `auth.kidsrelay.fr`, ou le jeton passe
// par le fragment et n'atteint jamais un serveur.

/// Chemin par defaut : celui impose par Supabase aujourd'hui. Le
/// changer suffit a servir cette page depuis un autre hebergeur.
export const CHEMIN_API_PAR_DEFAUT =
  '/functions/v1/consulter-partage';

export function construirePage(
  cheminApi: string = CHEMIN_API_PAR_DEFAUT,
): string {
  return `<!doctype html>
<html lang="fr">
<head>
